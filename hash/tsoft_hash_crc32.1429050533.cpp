...
uint32_t ts::hash::crc32::CRC32_TAB[256] = {
...
__int32 *lptab  = (__int32*)&CRC32_tab[0];
__asm {
 mov ESI,crc32
 mov EAX,[ESI]
 mov ESI,lptab
...
void __stdcall ts::hash::crc32::init_CRC32(void)
{
		const __int8 POLY_TAB[] = {0,1,2,4,5,7,8,10,11,12,16,22,23,26};
		uint32_t POLY_NOMINAL = 0x00000000L, n, k, c;
...
*/
uint32_t __stdcall ts::hash::crc32::calc_CRC32(const void *a_dst_ptr,uint32_t a_count)
{
		static uint32_t *lptab = (uint32_t*)&CRC32_TAB[0];
#ifdef BCC
	__asm {
		mov EDI,a_dst_ptr
		mov EDX,EDI
