...
//---------------------------------------------------------------------------
#ifndef __tsoft_bwt_matrix_HDR__
#define __tsoft_bwt_matrix_HDR__
//---------------------------------------------------------------------------
...
namespace compress {
//---------------------------------------------------------------------------
class  __bwt_transform {
private:
//---------------------------------------------------------------------------
public:
//---------------------------------------------------------------------------
	__bwt_transform(int32_t ablock);
	~__bwt_transform();
//---------------------------------------------------------------------------
	int32_t __stdcall generate(char *a_src_ptr, int32_t asrcLen);
		int32_t __stdcall sort(void);
	void __stdcall sort_column(int32_t acolumn,int32_t arow_bgn,int32_t arow_end);
	int32_t __stdcall Flush(char *a_dst_ptr);

	class __bwt_compressor {

	public:
		__bwt_compressor(int32_t ablock);
		~__bwt_compressor();
		...
		int32_t **index_ptr;
		int32_t  *index_row_ptr;
		int32_t  *index_of_first_row_ptr;
		int32_t **result_index_ptr;
...
#define NOT_EXIST_IN_TREE 0xFFFF0000L
			...
			__bwt_tree(int32_t ablock);
			~__bwt_tree();

			int32_t  *root_ptr;
			int32_t dim;
			int32_t  *last_inserted_ptr;
			int32_t  *leaf_count_ptr;
...
#define BGN 0
#define END 1
	void __stdcall find_groups(int32_t acolumn,int32_t arow_bgn,int32_t arow_end,int32_t *alist_bgn,int32_t *alist_end);
