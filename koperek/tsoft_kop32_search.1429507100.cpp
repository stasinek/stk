...
//---------------------------------------------------------------------------
__int32 __stdcall ts::file::__search::execute(const char *a_dir, const ts::__database* a_mask_list_ptr, ts::__database* a_output_list_ptr)
{
...
#define SEEKSRC_FIND_ADD_TO_list_ptr(path,found,bpl)\
		if (found && f_src_file.find_data.data.nFileSizeHigh==0)\
				{current = f_external_src_output_list_ptr->items()->add(CHAR_TO_DATABASE_ATOM(path));\
				 f_external_src_output_list_ptr->items()->set_number(current,IS,FILE_EXISTS);\
				 f_external_src_output_list_ptr->items()->set_number(current,FILE_CREATION_DATE,FILETIME_to_time_t(f_src_file.find_data.data.ftCreationTime));\
				 f_external_src_output_list_ptr->items()->set_number(current,FILE_MODIFIED_DATE,FILETIME_to_time_t(f_src_file.find_data.data.ftLastWriteTime));\
				 f_external_src_output_list_ptr->items()->set_number(current,FILE_ACCESSED_DATE,FILETIME_to_time_t(f_src_file.find_data.data.ftLastAccessTime));\
...
__int32 __stdcall ts::file::__search::f_seek_src(const char *a_lpSource)
{
...
__int32 __stdcall ts::file::__search::f_seek_src_file(const char *a_lpSource, const __int32 base_path_len)
{
		register __int32 current = 0;
		ts::__cstr_class *currentlpSource = new ts::__cstr_class(1024);//f_text_stack->push(1024);
		for (__int32 i = 0; i < f_external_mask_list_ptr->items()->count() && progress->cancel==0; i++) {
				currentlpSource->set(a_lpSource);
				currentlpSource->add("\\");
...
#define SEEKDST_FIND_ADD_TO_list_ptr(patch,found)\
		if (found && f_dst_file.find_data.data.nFileSizeHigh==0)\
				{current = f_external_dst_output_list_ptr->items()->add(CHAR_TO_DATABASE_ATOM(patch));\
				 f_external_dst_output_list_ptr->items()->set_number(current,IS, FILE_EXISTS);\
				 f_external_dst_output_list_ptr->items()->set_number(current,FILE_CREATION_DATE, FILETIME_to_time_t(f_dst_file.find_data.data.ftCreationTime));\
				 f_external_dst_output_list_ptr->items()->set_number(current,FILE_MODIFIED_DATE, FILETIME_to_time_t(f_dst_file.find_data.data.ftLastWriteTime));\
				 f_external_dst_output_list_ptr->items()->set_number(current,FILE_ACCESSED_DATE, FILETIME_to_time_t(f_dst_file.find_data.data.ftLastAccessTime));\
...
__int32 __stdcall ts::file::__search::create_destination_list(const char *a_lpDestination, const char *a_lpExtension_to_add, const char *a_lpExtension_to_delete, __database* a_output_list_ptr)
{
...
__int32 __stdcall ts::file::__search::f_seek_dst(const char *a_lpDestination, const char *a_lpExtension_to_add, const char *a_lpExtension_to_delete)
{
...
//
		...
		for (int  srclen_old = lpSource->len(), dstlen_old = lpDestination->len(); f_external_dst_output_list_ptr->items()->count() < f_external_src_output_list_ptr->items()->count() && progress->cancel==0; ) {
				...
				temp_lpSource->add((char*)((uint32_t)(char*)f_external_src_output_list_ptr->items()->get_text(f_external_dst_output_list_ptr->items()->count()) + srclen_old));
				// PREPARE NEW NAME
				if (len_of_ext_to_add > 0) {
