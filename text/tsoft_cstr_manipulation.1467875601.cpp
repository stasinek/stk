...
char     *__stdcall ts::cstr::allocex(size_t a_size, const char *a_src_ptr) {
return mov((char*)alloc(a_size), a_src_ptr); }
//---------------------------------------------------------------------------

char     *__stdcall ts::cstr::realloc(char *a_dst_ptr, size_t a_size) {
return (char*)ts::mem32::realloc(a_dst_ptr, a_size); }
//---------------------------------------------------------------------------

char     *__stdcall ts::cstr::reallocex(char *a_dst_ptr, size_t a_size, const char *a_src_ptr) {
...
char *__stdcall ts::cstr::insert(char *a_dst_ptr, const size_t astart, const char *ainsert)
{
#ifdef __DEBUG_CSTR__
__DEBUG_FUNC_CALLED("")
#endif
...
{
    return ts::mem32::chr(a_src_ptr,a_chr,a_end + 1);
}
//---------------------------------------------------------------------------
...
size_t __stdcall ts::cstr::chr_sum(const char* a_src_ptr, const char a_chr)
{
#ifdef __DEBUG_CSTR__
__DEBUG_FUNC_CALLED("")
#endif
...
if (src[0]=='\0')
return -1;
register size_t L_pos = chr(src,l) + 1;
if (L_pos<=0) return -1;
register size_t R_pos = L_pos + ts::cstr::chrr(src+L_pos, r) - 1;
if (R_pos< 0) return -1;
register size_t len = R_pos -L_pos + 1;
...
__DEBUG_FUNC_CALLED("")
#endif
    for (register size_t pos = 0, find, len_co = ts::cstr::len(a_seach_ptr), len_naco = ts::cstr::len(a_exch_ptr), lencel = ts::cstr::len(a_dst_ptr);; pos = find + len_naco) {
        if (a_dst_ptr[pos]=='\0')
            break;
...
__DEBUG_FUNC_CALLED("")
#endif
    size_t delimiter = ts::cstr::chr(a_src_ptr,'\\');
    if (delimiter>=0) {
...
__DEBUG_FUNC_CALLED("")
    ...
    register size_t len = ts::cstr::len(digits);
    register size_t sum = 0;
    for(register size_t i = len - 1; i >= 0; i--)
    {
      int temp = digits[i];
      if(alt) {
         temp *= 2;
...
size_t  __stdcall ts::cstr::compare(const char *alp1, const char *alp2)
{
#ifdef __DEBUG_CSTR__
__DEBUG_FUNC_CALLED("")
#endif
...
size_t  __stdcall ts::cstr::compare_end(const char *alp1, const char *alp2, const size_t aend)
{
#ifdef __DEBUG_CSTR__
__DEBUG_FUNC_CALLED("")
#endif
...
const char *__stdcall ts::cstr::itoa(const size_t aint)
{
#ifdef __DEBUG_CSTR__
__DEBUG_FUNC_CALLED("")
#endif
...
const char *__stdcall ts::cstr::itoa(const size_t aint, const int8_t abase)
{
#ifdef __DEBUG_CSTR__
__DEBUG_FUNC_CALLED("")
#endif
...
__DEBUG_FUNC_CALLED("")
#endif
ts::mem32::set((char*)alpdst+astart,a_znak,aend-astart);
return alpdst;
}
//---------------------------------------------------------------------------
