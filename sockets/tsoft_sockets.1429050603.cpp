...
// socket bez podawania IP lub na okreslone IP
		if (ts::cstr::cmpex(aipv4_addr_s,"0.0.0.0")==0) main_service.sin_addr.s_addr = INADDR_ANY;
	else main_service.sin_addr.s_addr = inet_addr( aipv4_addr_s );
	main_service.sin_port = htons( atoi(aipv4_port_s) );
...
HTTP_GOTO_ERROR_SOCKET:
	if (main_socket!=SOCKET_ERROR) close_socket(main_socket);
...
__int32 __stdcall ts::socket::start_simple_http_server(const char* path, const char *aipv4_addr_s, const char *aipv4_port_s)
{
	...
	buffer.send_done_count = send( main_socket, buffer.send_lp, ts::cstr::strlen(buffer.send_lp) + 1, 0 );
	while(true) {
		ts::mem32::set(buffer.recv_lp,0,MAX_REQUEST);
		buffer.recv_done_count = recv( main_socket,buffer.recv_lp,MAX_REQUEST,0 );
		if (buffer.recv_done_count == 0 || buffer.recv_done_count == WSAECONNRESET || buffer.recv_done_count  < 0 || buffer.recv_done_count == SOCKET_ERROR)
		...
		if (buffer.recv_done_count < 4) {
			...
			ts::cstr::substr_end(&temp_lp[ts::cstr::strlen(temp_lp)],buffer.recv_lp,4,temp_count);
			temp_count = ts::cstr::strlen(temp_lp);
			ts::cstr::replace_chr(temp_lp,'/','\\');
			ts::cstr::mov(buffer.send_lp,"HTTP/1.1 200 OK\r\n");
			ts::cstr::add(buffer.send_lp,"Date: Fri, 31 Dec 1999 23:59:59 GMT\r\n");
			ts::cstr::add(buffer.send_lp,"Server: tsoft/0.1\r\n");
			ts::cstr::add(buffer.send_lp,"Transfer-Encoding: identity\r\n");
			ts::cstr::add(buffer.send_lp,"Content-Type: ");
			ts::cstr::add(buffer.send_lp,(char*)ts::file::mime::decode((char*)temp_lp));
			ts::cstr::add(buffer.send_lp,"\r\n");
			ts::cstr::add(buffer.send_lp,"Content-Length: ");
			ts::cstr::add(buffer.send_lp,ts::socket::get_file_size_ansi(temp_lp));
			ts::cstr::add(buffer.send_lp,"\r\n");
			ts::cstr::add(buffer.send_lp,"\r\n");
			buffer.send_count = ts::cstr::len(buffer.send_lp);
