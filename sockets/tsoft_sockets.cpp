...
In some instances, it also refers to the current state of the socketï¿½for instance, calling accept on a socket that is not listening. \
");
        break;
        case WSAEMFILE:
//10024
...
Windows Sockets only allows a single blocking operationï¿½per- task or threadï¿½to be outstanding, and if any other function call is made (whether or not it references that or any other socket) the function fails with the case WSAEINPROGRESS error. \
");
        break;
        case WSAEALREADY:
//10037
        ts::console::print_formated("%s\n","Operation already in progress. \
An operation was attempted on a nonblocking socket with an operation already in progressï¿½that is, calling connect a second time on a nonblocking socket that is already connecting, or canceling an asynchronous request (case WSAAsyncGetXbyY) that has already been canceled or completed. \
");
        break;
        case WSAENOTSOCK:
//10038
...
Client applications usually need not call bind at allï¿½connect chooses an unused port automatically. \
...
Any other type of operation might also return this errorï¿½for example, setsockopt setting SO_KEEPALIVE if the connection has been reset. \
");
        break;
        case WSAESHUTDOWN:
//10058
...
This usually results from trying to connect to a service that is inactive on the foreign hostï¿½that is, one with no server application running. \
");
        break;
        case WSAELOOP:
//10062
...
An MX record is returned but no A recordï¿½indicating the host itself exists, but is not directly reachable. \
");
        break;
