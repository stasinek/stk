...
//---------------------------------------------------------------------------
static volatile int s_print_buffer_size = 0; static char *s_print_buffer = NULL;
//---------------------------------------------------------------------------
...
if (s_print_buffer_size==0) {
    ...
    s_print_buffer_size = VA_DEFAULT_SIZE; // vsnprintf nsize is passed without '\0' terminator
    ...
    int needed_size = ::vsnprintf(s_print_buffer,s_print_buffer_size-1,a_format,__va_param) + 1; //minimum needed
    va_end(__va_param);

    if (needed_size < VA_DEFAULT_SIZE) {
...
s_print_buffer[0]='\0';
for (int i = 0; i < f_buffer_needed; i+=a_text_len) { ts::mem32::mov((void*)&s_print_buffer[i],a_text,a_text_len);
    }
