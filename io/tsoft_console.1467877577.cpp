...
//---------------------------------------------------------------------------
static volatile uint32_t s_print_buffer_size = 0; static char *s_print_buffer = NULL;
//---------------------------------------------------------------------------
...
if (s_print_buffer_size==0) {
    ...
    s_print_buffer_size =VA_DEFAULT_SIZE-1;
...
#include <stdarg.h>
    ...
    uint32_t needed_size = ::vsnprintf(s_print_buffer,s_print_buffer_size,a_format,__va_param) + 8; //minimum needed
    va_end(__va_param);
...
{
ATOMIC(1)
int32_t a_text_len = ts::cstr::len(a_text);
...
s_print_buffer[0]='\0';
for (int32_t i = 0; i< an_times; i++) { ts::cstr::cat(s_print_buffer,a_text);
    }
ts::con::print(s_print_buffer);
ATOMIC_UNLOCK(1)
}
